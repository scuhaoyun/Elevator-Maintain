...
public class SwiftPages: UIView, UIScrollViewDelegate {
    ...
    override public func drawRect(rect: CGRect)
    {
        ...
        if (aeroEffectInTopBar) {
            scrollView = UIScrollView(frame: CGRectMake(0, topBarHeight, containerView.frame.size.width, containerView.frame.size.height))
        } else {
            scrollView = UIScrollView(frame: CGRectMake(0, topBarHeight, containerView.frame.size.width, containerView.frame.size.height))
        }
        ...
        let imgBack = UIImageView(frame: CGRectMake(0, 0, containerView.frame.size.width, topBarHeight))
        imgBack.image = UIImage(named: "pagesBarbak.png")
        topBar.addSubview(imgBack)
        ...
        [scrollView .setContentOffset(CGPointMake(pageSize.width * (CGFloat)(pageIndex), 0), animated: false)]
        oldX = animatedBar.frame.origin.x
    }
    ...
    public func loadPage(page: Int)
    {
        ...
        } else
        {
            ...
            newPageView = UIStoryboard(name: "Record", bundle: nil).instantiateViewControllerWithIdentifier(viewControllerIDs[page])
            newPageView.view.frame = frame
            newPageView.view.frame.size.height -= topBarHeight
    ...
    public func scrollViewDidEndDecelerating(scrollView: UIScrollView) {
        if oldX < animatedBar.frame.origin.x {
            pageIndex += 1
        }
        else if oldX > animatedBar.frame.origin.x {
            pageIndex -= 1
        }
        if pageIndex > viewControllerIDs.count - 1 {
            pageIndex = viewControllerIDs.count - 1
        }
        if pageIndex < 0 {
            pageIndex = 0
        }
