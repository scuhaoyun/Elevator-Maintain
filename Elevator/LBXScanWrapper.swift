...
class LBXScanWrapper: NSObject, AVCaptureVideoDataOutputSampleBufferDelegate , AVCaptureMetadataOutputObjectsDelegate  {
    ...
    init( videoPreView:UIView,objType:[String] = [AVMetadataObjectTypeQRCode],isCaptureImg:Bool,cropRect:CGRect=CGRectZero,success:( ([LBXScanResult]) -> Void) )
    {
        ...
        if session.canAddOutput(bufferOutput)
        {
            session.addOutput(bufferOutput)
        }
        ...
        bufferOutput.setSampleBufferDelegate(self, queue: dispatch_get_main_queue())
        ...
        if !CGRectEqualToRect(cropRect, CGRectZero)
        {
            ...
            bufferOutput.rectForMetadataOutputRectOfInterest(cropRect)
        }
...
//            }
           ...
           //let scanImg:UIImage? = UIImage(data: imageData)
                        ...
                        for idx in 0...self.arrayResult.count-1
                        {
                            self.arrayResult[idx].imgScanned = UIImage(CGImage: cgImage)
                        }
